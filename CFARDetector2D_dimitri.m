classdef (Sealed,StrictDefaults) CFARDetector2D_dimitri < phased.internal.AbstractDetector
%CFARDetector2D_dimitri   Constant false alarm rate detector for matrix data
%   H = CFARDetector2D_dimitri creates a constant false alarm rate (CFAR) detector
%   System object, H. This object performs CFAR detection on the matrix
%   input data.
%
%   H = phased.CFARDetector2D_dimitri(Name,Value) creates a CFAR detector object,
...
%
%   The algorithm used in CFARDetector2D_dimitri is cell averaging CFAR. Detection
...
%   equivalent.
%
%   CFARDetector2D_dimitri methods:
%
...
%
%   CFARDetector2D_dimitri properties:
%
...
%   cfar = phased.CFARDetector2D_dimitri('TrainingBandSize',2,'GuardBandSize',1,...
...
%#codegen
    ...
    methods
        ...
        function set.TrainingBandSize(obj,val)
            ...
            if ~any(val>0)
                error(message('phased:CFARDetector2D_dimitri:TrainingCellsCannotZero'));
            end
    ...
    methods
        function obj = CFARDetector2D_dimitri(varargin)
        ...
        function validateInputsImpl(obj,x,idx,varargin)
            ...
            if cond               
                 coder.internal.errorIf(cond, ...
                     'phased:CFARDetector2D_dimitri:XMustBe2D3DNonEmpty');
            end
            
            cond = ~ismatrix(idx) || ~isequal(size(idx,1),2) || isempty(idx);
            if cond
                coder.internal.errorIf(cond, ...
                    'phased:CFARDetector2D_dimitri:IDXMustTwoRows');
            end
            ...
            if cond
                coder.internal.errorIf(cond, ...
                    'phased:CFARDetector2D_dimitri:TrainingRegionLargerX');
            end
        ...
        function [yout,varargout] = stepImpl(obj,X,Idx,ThFac)
            ...
            for m = 1:numCUT
                if VI_CFAR
                    ...
                    %Dimitri VI-CFAR swag section
                    
                    %compute left and right means
                    left_mean=mean(X(trnindsl),1);
                    right_mean=mean(X(trnindsr),1);
                    
                    %compute left and right standard deviations
                    left_std=std(X(trnindsl));
                    right_std=std(X(trnindsr));
                    
                    %compute mean ratio
                    mean_ratio=right_mean/left_mean;
                    
                    %compute same means or no
                    if Kmr^-1 <= mean_ratio && Kmr >= mean_ratio
                        same_means=true;
                    else
                        same_means=false;
                    end
                    
                    %compute varitability index
                    left_VI=1+(left_std^2)/(left_mean^2);
                    right_VI=1+(right_std^2)/(right_mean^2);
                    ...
                    %if block to determine CA vs SO vs GO
                    if ~left_var && ~right_var % cases 1 & 2 none variability
                        if same_means %case 1 CA CFAR whole window
                            noisePowEst=mean([X(trnindsr);X(trnindsl)],1);
                            C1=C1+1;
                    ...
                    elseif left_var && right_var %case 3 both variable SOCFAR
                        noisePowEst = min(mean(X(trnindsl),1),mean(X(trnindsr),1));
                        C3=C3+1;
                    elseif ~left_var && right_var %case 4 leading window yes lagging window not variable CA on lagging window
                        noisePowEst=mean(X(trnindsl),1);
                        C4=C4+1;
                    elseif left_var && ~right_var %case 5 leading window no lagging window yes variable CA on leading window
                        noisePowEst=mean(X(trnindsr),1);
                        C5=C5+1;
                    end
                    
                    %compute left and right 
                    %noisePowEst = min(mean(X(trnindsl),1),mean(X(trnindsr),1));
                % end of dimitri vi-cfar block
                elseif obj.Method(1) == 'C' %'CA'
                    if m == 1
            ...
            C1%print counts of classifications
            C2
            C3
            C4
            C5
    ...
    methods (Static,Hidden,Access=protected)
        ...
        function header = getHeaderImpl
            header = matlab.system.display.Header(...
                'Title',getString(message('phased:library:block:CFARDetector2D_dimitriTitle')),...
                'Text',getString(message('phased:library:block:CFARDetector2D_dimitriDesc',...
                'CA','GOCA','SOCA','OS')));
        end
    end
    ...
    methods (Access = protected)
        ...
        function validateCUTS(obj,X,Idx)
            ...
            if cond 
                coder.internal.errorIf(cond, ...
                    'phased:CFARDetector2D_dimitri:TrainingCellsOutsideX');
            end
