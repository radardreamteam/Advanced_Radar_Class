       ...
       %% Determine Beam Direction
        ...
        if(clutter_contamination == 1)
            for k4 = 1:length(clutterSigPower)
                ...
                clutter_temp(clutterSig_Delay(k4)+1:clutterSig_Delay(k4)+sigLength) = sqrt(10^(((clutterSigPower(k4)+PAT(clutterSig_Doa(2,k4)+21,clutterSig_Doa(1,k4)+81)))/10))*dirPath(1:sigLength)*exp(j*phaseOffset).*exp(j*2*pi*clutterSig_Doppler(k4)*taxis(1:sigLength));
