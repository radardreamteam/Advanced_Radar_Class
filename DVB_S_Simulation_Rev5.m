...
% 1 - element: element-by-element
% 2 - beamform: beamformed
...
switch whichCancellation
    ...
    case 'beamform'
       ...
       %% Make Time Domain Signal
        ...
        Echos= refSignal';
        for k3 = 1:numTargets
            Echo{k3}(samp_offset(k3)+1:samp_offset(k3)+sigLength) = sqrt(10^((tgtPower_dBm(k3))/10))*dirPath(1:sigLength)*exp(j*phaseOffset).*exp(j*2*pi*FShift(k3)*taxis(1:sigLength));
       ...
       %% Determine Beam Direction
        ...
        % We must assume we know the doa of the direct path signal
        direct_distribute = collectPlaneWave(array,direct_reft,doas(:,1),samplingFreq);
    otherwise
...
switch whichDSIsuppression
    case 'NLMS'
        ...
        if (beamformedCancellation == 1)
          nlms = dsp.LMSFilter(filterOrder,'Method','Normalized LMS','StepSizeSource','Input port');
          [outputNLMS,errNLMS,weightsNLMS] = nlms(direct_reft,BeamformedSurv,0.001);
          DSIed = errNLMS;
        end
    case 'Wiener'
        filterOrder = 32;
        ...
        if (beamformedCancellation == 1)
          [outputW ,errW] = wienerf(filterOrder-1,direct_reft,BeamformedSurv);
          DSIed = errW;
        end
    case 'RLS'
        filterOrder = 32;
        ...
        if (beamformedCancellation == 1)
          [outputRLS ,errRLS] = RLSFilter(filterOrder-1,direct_reft,BeamformedSurv,FF,IP);
          DSIed = errRLS;
        end
    case 'FBLMS'
        filterLength = 768;
