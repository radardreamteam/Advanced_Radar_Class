...
function [RFSignal, timeVector] = DVBgenerate(cyclesPerSymbol, sampsPerCycle, fc,numFrames,pilotOn)
...
DVBstream = zeros(sampsPerSymbol*(length(finalOut)),1);

for i = 1:length(finalOut)
    for k = (i-1)*sampsPerSymbol+1:i*sampsPerSymbol
        DVBstream(k) = finalOut(i);
    end
end

Ls  = length(DVBstream);

% DVB-S baseband signal  
...
RFSignal = DVBstream'.*exp(2*pi*j*fc*timeVector); 
end

% 2GHz for the carrier wave
